...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (pokemonNameRaw) {
            ...
            const matchedKey = Object.keys(repelPknData).find(key => this.standardizeName(key) === standardizedInputName);
            ...
            const matchedKey = Object.keys(repelLocData).find(key => this.standardizeName(key) === standardizedInputName);
    ...
    chunkData(data, type, rawName) {
        ...
        data.forEach(detail => {
            ...
            if (counter === 3 || data.indexOf(detail) === data.length - 1) {
                chunkedEmbeds.push({
                    title: `${type === 'pokemon' ? 'Repel-Trick for Pokemon' : 'Repel-Trick in Location'} ${rawName}`,
